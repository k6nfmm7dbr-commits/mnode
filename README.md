...
| 面板实时/连接数 | 并行下载时速率峰值 5.32MB/s、停止后回落、TCP/UDP 会话数 | 全通过 |
...
| 交互菜单 | 全部菜单项（含面板菜单）、确认/取消、随机端口、无效输入、stdin 耗尽 | 61/61 |
| 运行时韧性 | 令牌鉴权、stop/start、内核与面板 `kill -9` 自愈、计数规则被清后重建且历史不清零 | 35/35 |
| 面板联动 | 改端口规则换代累计不丢、删节点其它累计保留、pid 不复用不串号 | 32/32 |
