...
DEFAULT_CONF = {
    ...
    "interval": 2,
    "tz": "Asia/Shanghai",
    # mihomo RESTful API：连接数从这里取（按 inbound 归属，含 UDP-over-TCP 会话）。
    # 留空则退回读 /proc/net/{tcp,udp}[6]。
    "core_api": "http://127.0.0.1:19090",
    "core_secret": "",
}
...
# --------------------------------------------------------------------------
# 连接数（首选）：mihomo RESTful API /connections
#
# 内核 /proc/net 只能看到"内核可见的 socket"。mihomo 的 VLESS/SS 都把客户端的
# UDP 流量封装在同一条 TCP 连接（UDP-over-TCP）或单个 UDP socket 里多路复用，
# 因此 /proc 数不出真实会话数。mihomo 自己的 API 按 inbound 名字给出每条逻辑
# 连接及其 network(tcp/udp)，这才是用户理解的"连接数"。
# API 不可用时自动退回 /proc 方案。
# --------------------------------------------------------------------------

_CORE_FAIL_UNTIL = [0.0]        # API 连续失败时的退避截止时间


def _core_api_url(conf):
    base = str(conf.get("core_api") or "").strip().rstrip("/")
    return (base + "/connections") if base else ""


def core_conns(conf, nodes):
    """
    从 mihomo API 读连接，按 inbound 名字归属到节点。
    返回 {node_id: {"tcp": int, "udp": int}}，失败返回 None。
    """
    url = _core_api_url(conf)
    if not url or time.monotonic() < _CORE_FAIL_UNTIL[0]:
        return None
    req = urllib.request.Request(url)
    secret = str(conf.get("core_secret") or "")
    if secret:
    ...
    try:
        ...
        _CORE_FAIL_UNTIL[0] = time.monotonic() + 30    # 30 秒内不再重试
        return None
    _CORE_FAIL_UNTIL[0] = 0.0

    # 节点名 / 端口 → node_id。mihomo 的 inboundName 就是 listeners 里的 name。
    by_name, by_port = {}, {}
    for n in nodes:
        nid = n["id"]
        ...
        for lo, hi in parse_ports(n):
            for p in range(lo, min(hi, lo + 4096) + 1):
                by_port[p] = nid

    out = {n["id"]: {"tcp": 0, "udp": 0} for n in nodes}
    for c in (doc.get("connections") or []):
        m = c.get("metadata") or {}
        nid = by_name.get(str(m.get("inboundName") or ""))
        if nid is None:
            try:
                nid = by_port.get(int(m.get("inboundPort") or 0))
            except (TypeError, ValueError):
                nid = None
        if nid is None or nid not in out:
            continue
        key = "udp" if str(m.get("network") or "").lower() == "udp" else "tcp"
        out[nid][key] += 1
    return out


def node_conns(conf, nodes):
    """连接数统一入口：先用 mihomo API，失败退回内核 /proc。"""
    got = core_conns(conf, nodes)
    if got is not None:
        return got
    return count_conns(nodes)
